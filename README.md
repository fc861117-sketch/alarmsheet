...
若其他裝置無法用同一組帳號密碼登入，請確認 Apps Script 已貼上最新版 `APPS_SCRIPT_CODE.gs`，並且已重新部署 Web App。最新版檔案上方會顯示 `Version: 2026-06-19-5`。
...
若 `sharedPasswordHash` 不小心填成一般明碼密碼，`Version: 2026-06-19-5` 會在網站讀取 GAS 設定時自動轉成雜湊值，不需要手動計算。

...
- CSV 匯出供統計使用
- JSON 匯出與匯入供備份使用
- 匯入舊 Excel `.xlsm/.xlsx/.csv` 清單並同步到 Google Sheet
- 以一組共用帳號密碼登入後才能使用系統
...
若其他裝置無法用同一組帳號密碼登入，請確認 Apps Script 已貼上最新版 `APPS_SCRIPT_CODE.gs`，並且已重新部署 Web App。最新版檔案上方會顯示 `Version: 2026-06-19-6`。

登入畫面會顯示目前連到的 GAS 版本。若顯示「未回傳版本」或不是 `2026-06-19-6`，代表網頁打到的 Web App 仍是舊部署。請在 Apps Script 的「管理部署作業」更新原本那個 Web App 部署；若是建立了新的部署，必須把新的 Web App URL 重新填入 `app.js` 的 `CLOUD_API_PARTS` 後再推送。

若登入畫面顯示「雲端連線失敗」，請點登入畫面的「開啟雲端測試」。正常情況會看到類似 `cloudTest({"ok":true,"hasCredentials":true,"version":"2026-06-19-6"})`。若測試連結也打不開，請在該裝置關閉內容阻擋、廣告阻擋或換一個網路後再試。
...
若 `sharedPasswordHash` 不小心填成一般明碼密碼，`Version: 2026-06-19-6` 會在網站讀取 GAS 設定時自動轉成雜湊值，不需要手動計算。
...
1. 先確認 Apps Script 已更新到 `Version: 2026-06-19-6` 並重新部署。
2. 登入網站後進入「設定」。
3. 在「匯入舊 Excel 清單」選擇原本的 `.xlsm` 檔。
4. 系統會讀取 `3.資料彙整` 工作表，顯示找到的筆數。
5. 確認後會合併到網站清單，並同步到 Google Sheet。

相同「序號」會以同一筆舊資料更新，不會重複新增。
